...
def group_by_centroid(restaurants, centroids):
    ...
    """
    # BEGIN Question 4
    # pairs = [find_closest(restaurant) for restaurant in restaurants] 
...
def find_predictor(user, restaurants, feature_fn):
    ...
    b = (S_xy)/(S_xx)
    a, r_squared = (mean(ys) - b *(mean(xs))) , pow(S_xy,2)/(S_xx*S_yy)
...
def best_predictor(user, restaurants, feature_fns):
    ...
    for func in feature_fns: 
        lst.append(find_predictor(user, reviewed, feature_fns))'''
    lst = [find_predictor(user, reviewed, func) for func in feature_fns]
...
def rate_all(user, restaurants, feature_fns):
    ...
    for restaurant in restaurants:
        if restaurant in reviewed: 
            rate_dic[restaurant_name(restaurant)] = user_rating(user,restaurant_name(restaurant))
        else: 
            rate_dic[restaurant_name(restaurant)] = predictor(restaurant)
    return rate_dic
